            ...
            <tbody>
                @foreach($list as $row)
                    <tr>
                        <td>{{$row->name}}</td>
                        <td class="text-right">
                            <a href="{{adminUrl('lembaga/edit')}}/{{$row->id}}?return_url={{request()->fullUrl()}}" class="btn btn-xs btn-secondary border-2 ml-1">Edit</a>
                            <a href="javascript:;" onclick="doDelete('{{adminUrl("lembaga/delete/".$row->id.'?return_url='.request()->fullUrl())}}')" class="btn btn-xs btn-dark border-2 ml-1">Hapus</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
