...
@section('content')
    ...
    <div class="card">
        <div class="card-header header-elements-inline">
            <h5 class="card-title">Daftar Privileges</h5>
            ...
            <tbody>
                @foreach($list as $row)
                    <tr>
                        <td>{{$row->name}}</td>
                        <td class="text-right">
                            <a href="{{adminUrl('privileges/edit')}}/{{$row->id}}?return_url={{request()->fullUrl()}}" class="btn btn-xs btn-secondary border-2 ml-1">Edit</a>
                            <a href="javascript:;" onclick="doDelete('{{adminUrl("privileges/delete/".$row->id.'?return_url='.request()->fullUrl())}}')" class="btn btn-xs btn-dark border-2 ml-1">Hapus</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
